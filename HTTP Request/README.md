...
The output file contains the output for all records passed through the step in json format. A key will indecate the record number. For example, if the step had three input records the format of the file will look like this: ```{"1": "--http result for rec 1--","2": "--http result for rec 2--","3": "--http result for rec 3--"}```
